...
TEST_F(AlpacaWSMarketFeedTest, ConnectsToFAKEPACAStream)
{
    ...
    AlpacaWSMarketFeed::config config{.api_key = api_key, .api_secret = api_secret, .test_mode = true};
    ...
    while (!received_bar && (std::chrono::steady_clock::now() - start_time) < timeout)
    {
...
TEST_F(AlpacaWSMarketFeedTest, HistoricalDataFeedTest)
{
    ...
    const AlpacaWSMarketFeed::config config{
        .api_key = "test_key", .api_secret = "test_secret", .host = "localhost", .port = "8765", .test_mode = false};
    ...
    auto connection = feed.connect_bar_handler(
        [&](const Bar1min& b)
        {
            ...
            std::stringstream ss;
            ss << "Bar{symbol: " << b.symbol() << ", open: " << b.open() << ", high: " << b.high()
    ...
    while (received_bars.load() < 10 && (std::chrono::steady_clock::now() - start_time) < timeout)
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    ...
    EXPECT_EQ(received_bars.load(), 10) << "Should have received exactly 10 bars";
    EXPECT_EQ(bar_strings.size(), 10) << "Should have 10 bar strings";

    for (size_t i = 0; i < bar_strings.size(); ++i)
    {
        EXPECT_FALSE(bar_strings[i].empty()) << "Bar string " << i << " should not be empty";
