  ...
  if (snapshot_history.size() >= 2)
    {
      ...
      values_changed
        |= (first_snapshot.at("EMA").at("ema") != last_snapshot.at("EMA").at("ema"));
      values_changed
        |= (first_snapshot.at("ATR").at("atr") != last_snapshot.at("ATR").at("atr"));
      values_changed
        |= (first_snapshot.at("MACD").at("macd") != last_snapshot.at("MACD").at("macd"));

      EXPECT_TRUE(values_changed)
        << "Indicator values did not change over time";
    }
