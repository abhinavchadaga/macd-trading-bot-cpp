...
protected:
  ...
  static void
  logComparison (const std::string &indicator, const int period,
                 const double my_result, const double talib_result,
...
TEST_F (IndicatorTest, EMA_CompareWithTALib)
{
  ...
  const TA_RetCode ret_code{ TA_EMA (
      ...
      period, &out_begin, &out_nb_element, talib_ema.data ()) };
...
TEST_F (IndicatorTest, ATR_CompareWithTALib)
{
  ...
  const TA_RetCode ret_code{ TA_ATR (
      0, static_cast<int> (close_prices.size () - 1), high_prices.data (),
      low_prices.data (), close_prices.data (), period, &out_begin,
...
TEST_F (IndicatorTest, MACD_CompareWithTALib)
{
  ...
  logComparison ("MACD-Histogram", 0, my_histogram_value,
                 talib_histogram_value, threshold);
  ...
  double histogram_abs_diff{ std::abs (my_histogram_value
                                       - talib_histogram_value) };
