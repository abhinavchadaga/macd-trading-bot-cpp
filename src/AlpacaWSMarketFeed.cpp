...
void AlpacaWSMarketFeed::start()
{
    ...
    const WebSocketSessionConfig ws_config{
        .host = host, .port = port, .endpoint = endpoint, .auth_msg = "", .sub_msg = "", .ssl_ctxt = _ssl_context};

    _ws_session =
        WebSocketSession::create(_ioc, ws_config, [this](const std::string_view frame) { on_websocket_frame(frame); });

    _ws_session->start();
}

void AlpacaWSMarketFeed::stop() const
{
    if (_ws_session)
    {
        _ws_session->stop();
...
void AlpacaWSMarketFeed::subscribe_to_bars(const std::vector<std::string>& symbols)
{
...
void AlpacaWSMarketFeed::subscribe_to_all_bars()
{
    _subscribed_symbols = {"*"};
    if (_authenticated && _ws_session)
    {
...
boost::signals2::connection AlpacaWSMarketFeed::connect_bar_handler(const bar_signal_t::slot_type& handler)
{
    return _bar_signal.connect(handler);
}

void AlpacaWSMarketFeed::on_websocket_frame(std::string_view frame)
{
    try
    {
        ...
        for (const auto& json_msg : json_array)
        {
            ...
            if (const std::string msg_type = json_msg["T"]; msg_type == "success" && json_msg.contains("msg"))
            {
...
void AlpacaWSMarketFeed::send_auth_message()
{
    nlohmann::json auth_msg = {{"action", "auth"}, {"key", _config.api_key}, {"secret", _config.api_secret}};
...
void AlpacaWSMarketFeed::send_subscription_message()
{
    const nlohmann::json sub_msg = {{"action", "subscribe"}, {"bars", _subscribed_symbols}};
...
void AlpacaWSMarketFeed::parse_bar_message(const nlohmann::json& message)
{
    try
    {
...
std::string AlpacaWSMarketFeed::get_websocket_url() const
{
    if (_config.test_mode)
