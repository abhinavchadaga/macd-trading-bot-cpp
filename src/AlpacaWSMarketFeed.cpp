...
void AlpacaWSMarketFeed::start() {
  ...
  _ws_session = WebSocketSession::create(
      _ioc, ws_config,
      [this](const std::string_view frame) { on_websocket_frame(frame); });
...
void AlpacaWSMarketFeed::subscribe_to_bars(
...
boost::signals2::connection AlpacaWSMarketFeed::connect_bar_handler(
...
void AlpacaWSMarketFeed::send_subscription_message() {
  const nlohmann::json sub_msg = {{"action", "subscribe"},
