...
void
BarAggregator::add_bar (const bar &input_bar)
{
  if (should_start_new_aggregation (input_bar.time ()))
    {
      ...
      _current_aggregated_bar = bar{ input_bar.symbol (), input_bar.open (),
...
BarAggregator::get_aggregation_boundary (
    const bar::timestamp_t &timestamp) const
{
  auto time_point = std::chrono::time_point_cast<std::chrono::minutes> (
      timestamp);
  ...
  return std::chrono::sys_time<std::chrono::nanoseconds>{
    std::chrono::minutes{ aligned_minutes }
  };
}