...
void BarAggregator::on_bar(const Bar& input_bar) {
  ...
  if (_bars_in_current_window == 0) {
    ...
    _current_aggregated_bar = Bar{current.symbol(),
                                  current.open(),
...
boost::signals2::connection BarAggregator::connect_aggregated_bar_handler(
    const aggregated_bar_signal_t::slot_type& handler) {
  return _aggregated_bar_signal.connect(handler);
}

void BarAggregator::emit_aggregated_bar() {
