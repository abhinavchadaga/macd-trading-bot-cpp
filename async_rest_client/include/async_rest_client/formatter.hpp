...
void format_request_line(fmt::memory_buffer& buf, const Request& req)
{
    fmt::format_to(
...
template<typename Request>
void format_request_headers(fmt::memory_buffer& buf, const Request& req)
{
    bool first_header = true;
    for (const auto& field : req)
    {
        if (const auto name{field.name_string()};
            name == "Host" || name == "host" || name == "Content-Type" || name == "content-type" ||
            name == "Content-Length" || name == "content-length" || name == "Authorization" || name == "authorization")
        {
            if (!first_header)
            {
            ...
            {
                fmt::format_to(std::back_inserter(buf), "{}: {}", name, field.value());
            }
            first_header = false;
        }
    }
...
struct fmt::formatter<http::request<http::string_body>>
{
    ...
    auto format(const http::request<http::string_body>& req, FormatContext& ctx) const
    {
        memory_buffer buf{};
...
struct fmt::formatter<http::request<http::empty_body>>
{
    ...
    template<typename FormatContext>
    auto format(const http::request<http::empty_body>& req, FormatContext& ctx) const
    {
        memory_buffer buf{};
...
struct fmt::formatter<http::response<http::string_body>>
{
    ...
    template<typename FormatContext>
    auto format(const http::response<http::string_body>& res, FormatContext& ctx) const
    {
        memory_buffer buf{};
        ...
        {
            if (const auto name{field.name_string()}; name == "Content-Type" || name == "content-type" ||
