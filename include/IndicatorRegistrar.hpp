...
struct IndicatorRegistrar
{
    IndicatorRegistrar()
    {
        IndicatorRegistry<IndicatorInterface>::register_indicator(
            ...
            { return std::make_pair(DerivedIndicator::name, std::make_unique<DerivedIndicator>(config)); });
    }
};

#define REGISTER_INDICATOR(Indicator, IndicatorInterface)                                                              \
    static IndicatorRegistrar<Indicator, IndicatorInterface> _registrar##Indicator;
