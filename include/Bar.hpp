...
concept ChronoDuration =
    ...
    std::same_as<T, std::chrono::weeks> ||
...
template <std::size_t Count1,
          ChronoDuration TimeUnit1,
          std::size_t Count2,
          ChronoDuration TimeUnit2>
bool isConsecutive(const Bar<Count1, TimeUnit1>& left,
  ...
  auto right_start =
      std::chrono::time_point_cast<std::chrono::nanoseconds>(right.timestamp());
  auto right_end =
      right_start +
