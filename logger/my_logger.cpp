...
void init_logger(const std::string_view name)
{
...
std::shared_ptr<spdlog::async_logger> make_logger(std::string_view name)
{
    ...
    const auto file_sink{
        std::make_shared<spdlog::sinks::daily_file_sink_st>(std::format("logs/{}/{}.logfile", name, name), 23, 59)};
    file_sink->set_level(spdlog::level::trace);
    sinks.push_back(file_sink);

    auto logger{std::make_shared<spdlog::async_logger>(
        std::string{name}, sinks.begin(), sinks.end(), spdlog::thread_pool(), spdlog::async_overflow_policy::block)};
